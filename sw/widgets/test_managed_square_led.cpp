...
/**
 * @brief Example of final implementation of UIModelObject
 * 
 */
...
public:
    ...
    test_square_led_model(std::string name);
...
/**
 * @brief Example of final implementation of w_SquareLed
 * 
 */
class test_managed_square_led_widget : public w_SquareLed
{
private:
    /// @brief the link to the actual test_square_led_model
    test_square_led_model *actual_displayed_model = nullptr;

public:
    test_managed_square_led_widget(test_square_led_model *actual_displayed_model,
...
/**
 * @brief the example implementation of UIObjectManager
 * 
 */
...
int main()
{
    ...
    display.show();
    /// 3- create 3 test_square_led_model  as displayed object for test_managed_square_led_widget
    test_square_led_model test_model_1 = test_square_led_model("TM1");
    test_square_led_model test_model_2 = test_square_led_model("TM2");
    test_square_led_model test_model_3 = test_square_led_model("TM3");
...
void test_square_led_model::process_control_event(ControlEvent _event)
{
     ...
     * They both toggle the boolean a_bool_value of the test_square_led_model, and set the change flag in order to trig the effective display.
     * 
     */
    switch (_event)
    {
...
test_managed_square_led_widget::test_managed_square_led_widget(test_square_led_model *actual_displayed_model,
                                               UIDisplayDevice *display_screen,
                                               size_t width,
                                               size_t height,
                                               uint8_t widget_anchor_x,
 ...
 *  - if the model is active, the led stops blinking and follows the value of a_bool_value/
 *  - if the model is waiting, the led stops blinking and represents the value of a_bool_value, 
 */
void test_managed_square_led_widget::draw_refresh()
{
    assert(this->actual_displayed_model != nullptr);
    {
        ...
        else
        {
            ...
            {
                if (this->actual_displayed_model->a_bool_value)
                {
                    rect(0, 0, frame_width, frame_height, true, FramebufferColor::WHITE);
                }
                else
...
test_square_led_model::test_square_led_model(std::string _name)
    : UIModelObject()
{
    this->name = _name;
}
...
void test_Manager::process_control_event(ControlEvent _event)
{
    ...
    case ControlEvent::RELEASED_AFTER_SHORT_TIME:
        if (current_active_model == this)
        {
            ...
            printf("%s is active \n", ((test_square_led_model *)this->current_active_model)->name.c_str());
        }
        else
        {
            make_manager_active();
            printf("manager is active, %s has focus \n", ((test_square_led_model *)this->managed_models[this->value])->name.c_str());
        }
        break;
    case ControlEvent::INCREMENT:
        if (current_active_model == this)
