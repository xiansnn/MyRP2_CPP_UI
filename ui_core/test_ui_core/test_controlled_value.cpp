...
Test_ControlledValue::Test_ControlledValue(std::string _name, int _min_value, int _max_value, bool _is_wrapable, int increment)
    : UIControlledIncrementalValue(_min_value, _max_value, _is_wrapable, increment)
{
    this->name = _name;
}

Test_ControlledValue::~Test_ControlledValue()
{
}

void Test_ControlledValue::process_control_event(ControlEvent _event)
{
    switch (_event)
    {
...
std::string Test_ControlledValue::get_name()
{
    return name;
}
